...
def calculate_histogram(image):
    """Verilen görüntünün histogramını hesaplar."""
    hist, _ = np.histogram(image.flatten(), bins=256, range=[0, 256])
    return hist

def calculate_cdf(hist):
    """Histogramdan kumulatif dağılım fonksiyonunu (CDF) hesaplar."""
    cdf = hist.cumsum()
    cdf_normalized = cdf * hist.max() / cdf.max()
    return cdf_normalized

def print_table(table, title, col1_name, col2_name):
...
image = np.array(data).reshape((32, 32))
...
hist_table = np.column_stack((np.arange(256), hist))
cdf_table = np.column_stack((np.arange(256), cdf))

print_table(hist_table, "Histogram Tablosu:", "Pixel Değeri", "Frekans")
print("\n")
print_table(cdf_table, "CDF Tablosu:", "Pixel Değeri", "Kumulatif Frekans")
